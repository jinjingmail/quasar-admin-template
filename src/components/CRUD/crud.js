...
function CRUD(options) {
  ...
  const data = {
    ...
    status: {
      ...
      // 添加或编辑状态
      get cu() {
        if (this.add === CRUD.STATUS.NORMAL && this.edit === CRUD.STATUS.NORMAL && this.view === CRUD.STATUS.NORMAL) {
          return CRUD.STATUS.NORMAL
        } else if (this.add === CRUD.STATUS.PREPARED || this.edit === CRUD.STATUS.PREPARED || this.view === CRUD.STATUS.PREPARED) {
  ...
  const methods = {
    ...
    toView(data) {
      crud.resetForm(JSON.parse(JSON.stringify(data)))
      if (!callVmHook(crud, CRUD.HOOK.beforeToView, crud.form)) {
        return
      }
    ...
    cancelCU() {
      ...
      if (viewStatus === CRUD.STATUS.PREPARED) {
        if (!callVmHook(crud, CRUD.HOOK.beforeViewCancel, crud.form)) {
          return
        }
        crud.status.view = CRUD.STATUS.NORMAL
      }
      ...
      if (viewStatus === CRUD.STATUS.PREPARED) {
        callVmHook(crud, CRUD.HOOK.afterViewCancel, crud.form)
      }
      // 清除表单验证
      if (crud.findVM('form').$refs.form) {
        crud.findVM('form').$refs.form.resetValidation()
      }
    },
    /**
     * 提交新增/编辑
    ...
    submitCU() {
      ...
      crud.findVM('form').$refs.form.validate().then(valid => {
        if (!valid) {
          return
        }
    ...
    // 选择改变
    selectionChangeHandler(details) {
      if (details && details.rows && details.rows.length) {
        crud.selections = details.rows
      } else {
        crud.selections = []
      }
    },
    ...
    resetForm(data) {
      ...
      // add by ghl 2020-10-04  页面重复添加信息时，下拉框的校验会存在，需要取消
      if (crud.findVM('form').$refs.form) {
        crud.findVM('form').$refs.form.resetValidation()
      }
    },
...
CRUD.HOOK = {
  ...
  /** 编辑取消 - 之前 */
  beforeViewCancel: 'beforeCrudViewCancel',
  /** 编辑取消 - 之后 */
  afterViewCancel: 'afterCrudViewCancel',
  /** 提交 - 之前 */
